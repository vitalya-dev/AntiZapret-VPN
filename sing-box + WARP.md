...
**Важно:** Мы пока не добавляем сервис в автозагрузку. Сначала нужно убедиться, что туннель поднялся и сервер не потерял доступ в сеть.
...
Если в ответ вы увидели строку `warp=on` или `warp=plus` — значит, туннель работает.

Только после успешной проверки добавляем сервис в автозагрузку, чтобы он запускался сам при перезагрузке сервера:
